...
const DeleteWishlistModal = ({handleToggle, open}: DeleteModalProps) => {
    const {userToken, wishlist, setWishlist, deleteWishlist} = useWishList() 
    ...
    return (
        <dialog id="delete-wishlist-modal" className="modal" open={open} onClose={handleToggle}>
            <div className="modal-box">
                <h3 className="font-bold text-lg">Delete wishlist</h3>
                <p className="py-4">Are you sure you want to delete this wishlist? This action cannot be reversed.</p>
                <div className="btn-wrapper float-right">
                    <button className="btn btn-primary mr-2" onClick={handleToggle}>Cancel</button>
